...
namespace ThuyTo.Areas.Admin.Controllers
{
    ...
    public class ProductController : Controller
    {
		...
		public async Task<IActionResult> UpdateBestSeller(int? IdToUpdate)
		{
			if (IdToUpdate == null)
			{
				return Json(new { message = "Error", status = 1 });
			}
			try
			{
				var item = await _context.Products
					.FirstOrDefaultAsync(m => m.ProductId == IdToUpdate );
				...
				if (result > 0)
				{
					...
					{
						status = 0,
						currentValue = item.IsBestSeller,
						message = "Cập nhật thành công"
					});
				}
				else
				{
					return Json(new { status = 4, message = "Không có thay đổi nào" });
				}
			}
			catch (Exception ex)
			{
				return Json(new { status = 3, message = "Lỗi server: " + ex.Message });
			}
		}
